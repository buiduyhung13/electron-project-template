  ...
  doScrapJob = async(e) => {
    ...
    if (filePath) {
      ...
      if (data.length > 0) {
        ...
        var succeedWriter = csvWriter({sendHeaders: true});
        var failedWriter = csvWriter({sendHeaders: true});
        ...
        var scrapData = (projectName, projectURL) => {
          return new Promise((resolve, reject) => {
            ...
            var fn = async() => {
              ...
              do {
                ...
                if (!searchResults.isBlocked) {
                  ...
                  if (lastPage === true) {
                    ...
                    await this.setState({projectScrappedSucceed: projectScrappedSucceed, propertySucceed: propertySucceed});
                  ...
                  if (this.state.projectScrappedFailed <= 0) {
                    ...
                    var isCaptchaResolved = await utils.byPassCaptcha(mainWindow);

                    if (isCaptchaResolved !== true && needHumanReaction <= 0) {
                      ...
                      logger.info(`Captcha is resolved or not display`);
                      if (isCaptchaResolved) {
                        this.state.totalCaptchaResolved = this.state.totalCaptchaResolved + 1;
                      }
                      selection = 2;
                    }
                    needHumanReaction--;
                  ...
                  if (selection === 1) {
                    ...
                    failedWriter.write({condoName: projectName, condoLink: projectURL});

                    var projectScrappedFailed = this.state.projectScrappedFailed + 1;
                    logger.info(`projectScrappedFailed: ${projectScrappedFailed}`);
                    this.setState({projectScrappedFailed: projectScrappedFailed})
                  } else {
                    lastPage = false;
                    page--;
              ...
              } while (lastPage === false)return totalPropertyItems;
            }
      ...
      this.setState({currentState: `Cannot read data, please check again!!!`, loading: false})
    }
  }
  ...
  parsePropertyInfo = (searchPageResult, projectName, projectURL, writer) => {
    return new Promise((resolve, reject) => {
      ...
      itemArrays.forEach(async(i) => {
        try {
          var leftInfoData = leftInfos
            .get(i)
            .children;
          var rightInfoData = rightInfos
            .get(i)
            .children;
          ...
          var VALUE = {
            ...
            guruProjectName: (listingInfo["projectName"] !== undefined
              ? listingInfo["projectName"]
              : NO_DATA),
            listedDate: getListedData(leftInfoData) !== NO_DATA
              ? getListedData(leftInfoData)["listedDate"]
              : NO_DATA,
            floorArea: getFloorArea(rightInfoData),
            askingPrice: getAskingPrice(askingPrice),
            listingId: (listingInfo["listingID"] !== undefined
              ? listingInfo["listingID"]
              : NO_DATA),
            agentName: getAgentName(leftInfoData),
            ...
            urlOfListing: (listingInfo["listingURL"] !== undefined
              ? listingInfo["listingURL"]
              : NO_DATA),
            ...
            relisted: getListedData(leftInfoData) !== NO_DATA
              ? getListedData(leftInfoData)["relisted"]
              : NO_DATA
          }
          ...
          this.setState({currentState: `Scrapping ${listingInfo.projectName} (${listingInfo.listingID}) failed`});
        }
      });
      resolve(results);
    })
    ...
    try {
      if (condoResultPage !== undefined) {
        ...
        if (isBlocked <= 0) {
          ...
          if (!isLastPage) {
            try {
              logger.info(lastPage[0].children[0].data);
              logger.info(lastPage[1].children[0].data);
            } catch (error) {}
              ...
              <Container>
                <Table celled>
                  ...
                  <Table.Body>
                    ...
                    <Table.Row>
                      <Table.Cell>
                        <Label color='red' basic size="large">Total captcha resolved</Label>
                      </Table.Cell>
                      <Table.Cell textAlign="center">
                        {this.state.totalCaptchaResolved}
                      </Table.Cell>
                    </Table.Row>
                    <Table.Row>
                      <Table.Cell>
...
const generateOutput = (app) => {
  ...
  return {succeedFile: succeedFile, failedFile: failedFile};
}
...
function getListingInfo(info) {
  try {
    ...
    listingInfo["projectName"] = data[0]
      .attribs["href"]
      .split("/")[
        data[0]
          .attribs["href"]
  ...
  return new Promise((resolve, reject) => {
    ...
    csv
      ...
      .on("data", function (data) {
        userData.push({projectName: data[0], projectLink: data[1]});
      })
      .on("end", function () {
        resolve(userData);
      })
